...
const FeaturedPost = ({ featuredPost, tooltip }) => (
  <section className={`featured ${featuredPost.type}`}>
    <a className="featured-post" href={featuredPost.url} name="navigate to featured post">
      ...
      <img alt="game header image" className="featured-image" src={require(`../../../images/headers/${featuredPost.image}`)} />
      {featuredPost.type !== 'spotlight' && <img alt="game header image" className="featured-image-side" src={require(`../../../images/headers/${featuredPost.image}`)} />}
    </a>
  </section>
)
